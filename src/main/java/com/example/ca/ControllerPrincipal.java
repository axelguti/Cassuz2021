...
import com.example.ca.DAOFactory.DAOFactory;
import com.example.ca.DTO.*;
import com.example.ca.Reportes.PromotorGanancia;
import com.example.ca.Util.Estado;
import com.example.ca.beans.IconTest;
...
public class ControllerPrincipal implements Initializable {
    ...
    private PedidoInterface entity;
    ...
    private void sumaTotalCaja(){
        ...
        Predicate<CajaDTO> pred;
        String fecha=calcularFormatoFecha();
        String f0="0"+fecha;
        if(LocalDateTime.now().getDayOfMonth()<=9){
            pred=a->a.getFecha().equalsIgnoreCase(f0);
        }else{
            pred=a->a.getFecha().equalsIgnoreCase(fecha);
        }
        double sumacaja;
    ...
    private void mostrarlistaEstado(){
        ...
        tcPedidosEstado.setCellValueFactory(new PropertyValueFactory<>("es"));
        tcPedidosEstado.setCellFactory(ComboBoxTableCell.forTableColumn(espedidos));
        tcPedidosEstado.setOnEditCommit(edit->{
            edit.getRowValue().setEs(edit.getNewValue());
            entity.modificarEstado(edit.getRowValue());
        });
    ...
    private void cargardatos(){
        ...
        for(PedidosDTO p:listar){
            ...
            datos.addAll(new PedidosDTO(p.getDni(),p.getNombre(),p.getApellido(),p.getIdPedido(),p.getFechaPedido(),p.getNomCatalogo(),
                    p.getPagina(),p.getMarca(),p.getColor(),p.getPrecio(),p.getTalla(),p.getCodProducto(),p.getTipopago(),p.getBanco(),p.getEs()));
            System.out.println(p.getEs());
        }
        for (PedidosDTO p:listar){
            switch (p.getEs()){
                case "Proceso":{
                    estadoPedido.getSelectionModel().select("Proceso");
                }break;
                case "En Almacen":{
                    estadoPedido.getSelectionModel().select("En Almacen");
                }break;
                case "Entregado":{
                    estadoPedido.getSelectionModel().select("Entregado");
                }
            }
        }
